 ...
 *
 * The arithmetic is `src/shared/ocrPreprocess.js`; this is the canvas around
 * it. The split is so `scripts/ocr-variants.mjs` can measure a proposed variant
 * in Node against real captures — a preprocessing change that is only ever run
 * in a browser can only be judged by looking at it.
 *
 * @param {HTMLCanvasElement} source  an upscaled region crop
 * @param {object} [options]  see preprocessPixels
 */
export function preprocessForOcr(source, options = {}) {
  const { width, height } = source;
  const image = source.getContext('2d').getImageData(0, 0, width, height);
  const processed = preprocessPixels(image, options);

  const canvas = document.createElement('canvas');
  canvas.width = width;
  canvas.height = height;
  canvas.getContext('2d').putImageData(new ImageData(processed.data, width, height), 0, 0);
  return canvas;
}
