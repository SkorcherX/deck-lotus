 ...
 * Falls back to default avatar (initials) if no Gravatar found
 */
export function getGravatarUrl(email, size = 80) {
  if (!email) return null;

  // MD5 hash of lowercase email (Gravatar requirement)
  const hash = md5(email.toLowerCase().trim());

  // d=404 returns 404 if no gravatar, allowing us to detect and use fallback
  // d=mp uses a mystery person as default
...
export function getUserInitials(username) {
  ...
  // Take first 2 characters of username, uppercase
